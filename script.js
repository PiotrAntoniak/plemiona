    ...
    window.twLib = {
      ...
      queueLib: {
        ...
        addItem: function (item) {
          // FIX: was returning the minimum length value instead of the index of the least busy queue
          let leastBusyIdx = twLib.queues.map(q => q.length)
            .reduce((iMin, curr, i, arr) => curr < arr[iMin] ? i : iMin, 0);
          twLib.queues[leastBusyIdx].enqueue(item);
        },
...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  // fires after the current AJAX request fully resolves. This prevents the
  // double-loop race condition that caused skipped sends and a runaway progress bar.
  const processAutoSendQueue = function () {
    ...
    $('.autoSendButton').val(`Auto-sending... (${autoSendQueue.length} remaining)`);
  ...
  const buildTable = function (plan) {
    ...
    html += `<tr>
               <td colspan="4" style="text-align: center;">
                 <input type="button" class="btn autoSendButton" value="Auto-Send All Farms (200-400ms delay)" style="margin: 5px;">
               </td>
             </tr>`;
  ...
  const createPlanning = function (optionDistance, optionTime, optionMaxloot, data) {
    ...
    for (let prop in data.villages) {
      ...
      orderedFarms.forEach((el) => {
        ...
        // FIX: fallback to 'a' if 'b' template is not configured; skip entirely if neither exists
        let template = data.farms.templates[template_name] || data.farms.templates['a'];
        if (!template) return;
  ...
  // FIX: sendFarm now accepts an onDone callback.
  // This is what drives the auto-send chain forward — the next farm only starts
  // after this one's AJAX response (success or failure) has been handled.
  // The retry path also correctly threads the callback through so it isn't lost.
  const sendFarm = function ($this, onDone) {
    ...
    if (!farmBusy && !(Accountmanager.farm.last_click && n - Accountmanager.farm.last_click < 200)) {
      ...
      // Busy — retry after a short wait, preserving the onDone callback
      setTimeout(() => sendFarm($this, onDone), 500);
    }
  };
